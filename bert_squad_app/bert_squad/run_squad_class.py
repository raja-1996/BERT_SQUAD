...
class SquadDataset():
    def __init__(self, args):
        self.initilize_params(args)
        self.RawResult = collections.namedtuple("RawResult",
                                           ["unique_id", "start_logits", "end_logits"])

    def initilize_params(self, args):
    ...
    def read_context_and_question(self, paragraph, question):
        """Read a SQuAD json file into a list of SquadExample."""
    ...
    def train(self, args):
        ...
        if args.do_train:
            train_examples = self.read_squad_examples(
        ...
        if args.trained_model_path and args.do_predict:
            self.predict(args)



    def predict(self, args, paragraph=None, question=None):
        if args.trained_model_path and args.do_predict :
            output_model_file = os.path.join(args.trained_model_path , WEIGHTS_NAME)
            output_config_file = os.path.join(args.trained_model_path , CONFIG_NAME)
            config = BertConfig(output_config_file)
            model = BertForQuestionAnswering(config)
            model.load_state_dict(torch.load(output_model_file))
            model.to(self.device)
        ...
        if args.do_predict and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
            if paragraph and question:
                eval_examples = self.read_context_and_question(paragraph, question)
            else:
            ...
            for input_ids, input_mask, segment_ids, example_indices in tqdm(eval_dataloader, desc="Evaluating", disable=True):
            ...
            all_predictions = self.write_predictions(eval_examples, eval_features, all_results,
                              args.n_best_size, args.max_answer_length,
                              args.do_lower_case, output_prediction_file,
                              output_nbest_file, output_null_log_odds_file, args.verbose_logging,
                              args.version_2_with_negative, args.null_score_diff_threshold)

            print(list(all_predictions.values()))
...
def main():
    ...
    parser.add_argument("--trained_model_path", default=None, type=str,
                        help="Path of fine tuned model ")
    parser.add_argument("--command_line", action='store_true',
                        help="Enter paragraph and question from command line")
